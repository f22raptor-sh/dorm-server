...
form1.addEventListener('submit', async (event) => {
  ...
  const select1 = formData.get('select_1');
  const select2 = formData.get('select_2');
  if (select1 == '이외' && select2 == '옵션을 선택해 주세요') {
    showToast('상벌점을 선택해 주세요.', 2);
  } else {
    if (select1 == '이외') {
      var score = select2.split('점')[0];
      var log = select2;
    } else {
      var score = select1.split('점')[0];
      var log = select1;
    }
    const send_data = {
...
uploadButton.addEventListener('click', async () => {
  ...
  if (yes_no == 'y' || yes_no == 'Y') {
    ...
    try {
      ...
      if (response.ok) {
        showToast('성공적으로 초기화 되었습니다', 2);
      } else {
        showToast('파일 업로드 실패.', 3);
      }
    } catch (error) {
      console.log(error);
