...
export class PointService {
  ...
  // score: interger
  // log : 기록 텍스트
  // number, except : 띄워쓰기 또는 콤마 등으로 구분된 학번들

  async pointAll(state: string, score: string, log: string, except: string) {
  ...
  async pointSelect(state: string, score: string, log: string, number: string) {
    const today = new Date();
    const year = today.getFullYear();
    const month = String(today.getMonth() + 1).padStart(2, '0');
    const date = String(today.getDate()).padStart(2, '0');
  ...
  async authReset(std_num: string) {
    let updates = {};
    updates[std_num] = '';
